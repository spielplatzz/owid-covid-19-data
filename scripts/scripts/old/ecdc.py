...
from utils.slack_client import send_success, send_warning
...
def download_xlsx(last_n=2):
    ...
    for date in daterange:
        ...
        for ext in ["xlsx"]:
            os.system(
                "curl --silent -f -o %(DIR)s/%(filename)s https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-%(filename)s"
                % {"filename": filename + "." + ext, "DIR": RELEASES_PATH}
            )


def download_csv():
    os.system(
        "curl --silent -f -o %(DIR)s/latest.csv -L https://opendata.ecdc.europa.eu/covid19/casedistribution/csv/data.csv"
        % {"DIR": RELEASES_PATH}
    )
...
def _load_merged(filename):
    ...
    return df_data.merge(df_locs, how="left", on=["countriesAndTerritories"])
...
def check_data_correctness(filename):
    ...
    if df_uniq["location"].isnull().any():
        ...
        df_uniq[["countriesAndTerritories"]].drop_duplicates().rename(
        ...
        for var_name in ["cases", "deaths"]:
            ...
            if new_RA >= 1.5 * previous_RA and new_RA > 100:
                sudden_changes_msg += "<!> Sudden increase of *{}* in *{}*: {} (7-day average was {})\n".format(
                    var_name, location, int(country_vals[-1]), int(previous_RA)
                )
...
def discard_rows(df):
    ...
    df = df[-((df["date"] == df["date"].max()) & (df["location"].isin(["Europe", "European Union"])))]
    # Drop last two days for Belgium, to avoid artificially declining series due to reporting lags
    df = df[-((df["date"] > df["date"].max() - timedelta(days=2)) & (df["location"] == "Belgium"))]
    return df
...
def load_standardized(filename):
    df = (
        ...
        .rename(columns={"dateRep": "date", "cases": "new_cases", "deaths": "new_deaths"})
    )
...
def export(filename):
    ...
    df_loc = _load_merged(filename)[["countriesAndTerritories", "location"]].drop_duplicates()
    ...
    return standard_export(load_standardized(filename), OUTPUT_PATH, DATASET_NAME)
...
def main(filename=None, skip_download=False):
    ...
    send_success(channel="corona-data-updates", title="Updated ECDC GitHub exports")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--skip-download",
        action="store_true",
        help="Skip downloading files from the ECDC website",
    )
    args = parser.parse_args()
    main(filename=args.filename, skip_download=args.skip_download)
